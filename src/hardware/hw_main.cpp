...
#include "../k_kart.h"										//SCS - RADIO END
...
static void HWR_DrawSprite(gl_vissprite_t *spr)
{
...

	{
		...
		// Determine the blendmode and translucency value
		{
			...
			if (RR_ShouldGhostRing(spr->mobj) || RR_ShouldGhostRingboxes(spr->mobj) || RR_ShouldGhostItemCapsuleParts(spr->mobj)) {		//SCS - RADIO START
	...
	*/
	...
	if (ghostMo || isVoltageAura) {
		if (ghostMo) {
			...
			vis->colormap = R_GetTranslationColormap(TC_RAINBOW,static_cast<skincolornum_t>(static_cast<unsigned char>(sparkColor)), GTC_CACHE);
		}																						//SCS - RADIO END
	}
